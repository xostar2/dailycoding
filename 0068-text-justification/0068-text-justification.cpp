...
public:
    vector<string> fullJustify(vector<string>&words, int max_width) {
        ...
        while(i<n){
            ...
            while(i<n && total_characters + count + words[i].size()<=max_width){
            ...
            int equal_spaces = total_spaces/(count-1);
            int res_spaces = total_spaces%(count-1);

            vector<int>spaces(count-1,equal_spaces);
            int j=0;
            while(res_spaces){
                spaces[j++] +=1;
                res_spaces--;
